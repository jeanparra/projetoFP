...
def caixaSalvar(request):
    if request.method == 'POST':
        ...
        caixa.pessoa_id = request.POST.get('pessoa_id', '').upper()
        ...
        try:
            if textoBusca == 'TUDO':
                ...
                caixas = Caixa.objects.filter(
                    (Q(pessoa_id__contains=textoBusca) |
                    Q(tipo__contains=textoBusca) |  
                    Q(descricao__contains=textoBusca) | 
                    Q(valor__contains=textoBusca) | 
                    Q(pagseguro__contains=textoBusca) |
                    Q(data__contains=textoBusca))).order_by('-tipo')  #BUSCA POR NOME OU EMAIL OU TELEFONE OU LOGRADOURO... EORDENADO POR NOME.
        except:
            caixas = []
